...
public class RobotMover_COMPLETED : MonoBehaviour
{
    ...
    void UpdateBody()
    {
        //This function just LERPs between the start and end 
        //positions of our body for the current step.
        stepInfo.bodyTimer += Time.deltaTime;

        float t = Mathf.Clamp(stepInfo.bodyTimer / stepInfo.bodyStepTime, 0.0f, 1.0f);

        Vector3 pos = Vector3.Lerp(stepInfo.bodyStart, stepInfo.bodyEnd, t);
        pos.y = transform.position.y;

        transform.position = pos;

        //Likewise, we SLERP our rotation.
        Quaternion rot = Quaternion.Slerp(stepInfo.rotationStart, stepInfo.rotationEnd, t);
    ...
    {
        //This function just LERPs between the start and end 
        //positions of our current leg during this step.
        Leg leg = legs[stepInfo.curLegID];

        stepInfo.legTimer += Time.deltaTime;
        float t = Mathf.Clamp(stepInfo.legTimer / stepInfo.legStepTime, 0.0f, 1.0f);

        Vector3 targetPos = Vector3.Lerp(
            stepInfo.legData[stepInfo.curLegID].targetStart,
            stepInfo.legData[stepInfo.curLegID].targetEnd, t);

        //For our y-position, we'll create an arc - for 
        //the first half of our step time, we'll raise the leg.
        //For the second half, we'll lower the leg.
        targetPos.y = (t < 0.5) ? Mathf.SmoothStep(groundHeight, stepInfo.legStepHeight, 2.0f * t)
            : Mathf.SmoothStep(stepInfo.legStepHeight, groundHeight, 2.0f * (t - 0.5f));
